...
if rg -q 'published: "5432"|published: "8000"' /tmp/flow-monitor-prod-compose.yml; then
  fail "production compose exposes PostgreSQL or backend port"
fi
rg -q 'published: "80"' /tmp/flow-monitor-prod-compose.yml || fail "production compose does not expose 80"
rg -q 'published: "1883"' /tmp/flow-monitor-prod-compose.yml || fail "production compose does not expose 1883"
rg -q 'host_ip: 127.0.0.1' /tmp/flow-monitor-prod-compose.yml || fail "EMQX dashboard is not bound to 127.0.0.1"
